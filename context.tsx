...
export interface GooseCanvasProps {
  wallpaperSrc: string
   ...
   *  to the renderer via gooseTabSel (which uses pressedScale=78/56). */
   ...
   *  like gooseTglTarget / gooseTglDragStart / gooseTglDrag / gooseTglDragEnd. */
  rendererRef?: React.MutableRefObject<LiquidGlassRenderer | null>
  className?: string
  /** Device pixel ratio override (0 = use device DPR, capped at 1.5 by
   *  the renderer's gooseResize). Applied on renderer init + when it changes. */
  dpr?: number
...
export function GooseCanvas({
  ...
  // Module-level gooseDragGroups from catalog.tsx — tracks which toggle
  // groups are being dragged (gooseTglTarget skipped to avoid drift).
  ...
  // --- Init renderer + wallpaper + gooseResize observer ---
  React.useEffect(() => {
    ...
    renderer.gooseBG(backgroundColor)
    renderer.gooseLoadWP(wallpaperSrc).catch((e) => console.error(e))
    renderer.gooseLoadSDF('/clock_sdf.webp').catch((e) => console.error(e))
    ...
    const gooseResize = () => {
      ...
      renderer.gooseResize(r.width, r.height)
    }
    // Apply the DPR override BEFORE the first gooseResize so the renderer uses
    // the correct DPR from the start (otherwise gooseResize caps at 1.5).
    if (dpr != null) {
    ...
    gooseResize()
    const ro = new ResizeObserver(gooseResize)
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const cur = renderer.gooseGetScrollY()
      renderer.gooseScrollY(cur + delta)
    }
  ...
  React.useEffect(() => {
    rendererRefInternal.current?.gooseBG(backgroundColor)
  }, [backgroundColor])
  ...
  React.useEffect(() => {
    ...
    renderer.dpr = dpr > 0 ? Math.max(0.5, Math.min(deviceDpr, dpr)) : deviceDpr
    const r = containerRef.current?.getBoundingClientRect()
    if (r) renderer.gooseResize(r.width, r.height)
  }, [dpr])
  ...
  React.useEffect(() => {
    ...
    renderer.gooseReqRender()
  }, [cornerStyle])

  // Push the latest element list to the renderer.
  React.useEffect(() => {
    rendererRefInternal.current?.gooseElements(elements)
  }, [elements])

  // Push content height (for scroll clamping).
  React.useEffect(() => {
    if (contentHeight !== undefined) {
      rendererRefInternal.current?.gooseContentH(contentHeight)
    }
  }, [contentHeight])

  // Reset scroll when scrollResetToken changes (destination switch).
  React.useEffect(() => {
    rendererRefInternal.current?.gooseScrollY(0)
  }, [scrollResetToken])
  ...
  React.useEffect(() => {
    ...
    for (const [groupId, target] of Object.entries(toggleTargets)) {
      // Skip groups currently being dragged — gooseTglDrag controls their
      // fraction, gooseTglTarget would conflict and cause drift.
      if (gooseDragGroups.has(groupId)) continue
      r.gooseTglTarget(groupId, target)
    }
  }, [toggleTargets])

  // Sync tabTargets → renderer (programmatic tab selection, e.g. via tap).
  // Uses gooseTabSel which sets pressedScale=78/56 (vs toggle's 1.5).
  React.useEffect(() => {
    ...
    for (const [groupId, { tabIndex, tabsCount }] of Object.entries(tabTargets)) {
      ...
      // still animating). gooseTabSel would zero velocity and fight
      // the spring, causing the indicator to jump/snapping wrong.
      if (gooseDragGroups.has(groupId)) continue
      r.gooseTabSel(groupId, tabIndex, tabsCount)
    }
  }, [tabTargets])
  ...
  const handlePointerDown = React.useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      let hit: GooseElement | null = null
      for (let i = els.length - 1; i >= 0; i--) {
        ...
        let testX = x, testY = y
        const elRot = (el as GooseElement & { elementRotation?: number }).elementRotation
