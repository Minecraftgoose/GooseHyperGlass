 ...
 * ------------------------------------------------------------------ */
export function buildBottomTabs(W: number, H: number, onBack: () => void, state: CatalogState, setState: (patch: Partial<CatalogState> | ((prev: CatalogState) => Partial<CatalogState>)) => void, rendererRef: React.MutableRefObject<LiquidGlassRenderer | null> | null = null, palette: ThemePalette = LIGHT_PALETTE, tabsConfig?: Array<Array<{ icon: string; label: string }>>, single: boolean = false, second: boolean = false): CatalogResult {
  ...
  function buildTabBar(idPrefix: string, tabs: Array<{ icon: string; label: string }> | null, selectedTab: number, onSelect: (i: number) => void, y: number) {
    ...
    for (let i = 0; i < tabsCount; i++) {
      ...
      const cfg = tabs?.[i]
      const iconPath = cfg?.icon ?? FLIGHT_ICON_PATH
