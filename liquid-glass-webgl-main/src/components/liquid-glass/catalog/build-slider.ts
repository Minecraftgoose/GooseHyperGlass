...
export function buildSlider(
  ...
  const SLIDER_KNOB_HIT_H = 48 * DP // lifted: used by BOTH wall + card knob hit areas (const is block-scoped)
  if (drawWall) {
  ...
  // 白卡对照（仅当 drawCard）。
  let cardContentBottom = s1TrackY + SLIDER_KNOB_H + 24 * DP
  ...
  const cardY = drawWall ? (s1TrackY + SLIDER_KNOB_H + 16 + 24) : (24 * DP) // 只白卡时从顶部起排
  ...
  cardContentBottom = cardY + cardH + 24 * DP
  }
